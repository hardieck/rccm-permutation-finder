//
// Created by hardieck on 11/22/21.
//

#ifndef ADDNET_PERMUTATOR_V2_CALC_RCCM_C2_H
#define ADDNET_PERMUTATOR_V2_CALC_RCCM_C2_H
#include "calc_rccm_base.h"
#include "calc_selective_adder_typ_a.h"
#include "calc_selective_adder_typ_b.h"

class calc_rccm_C2 : public calc_rccm_base
{
public:
    virtual rccm_type type() const {return typ_C2;}
    calc_rccm_C2()
    {
    ...
    std::set<int>* compute(base_obj *parent_)
    {
        ...
        for (int i=((calc_selective_adder_base*)(parent->sel_add[1].calc))->get_spec().input_count_A; i < input_count;++i)
        {
            IF_VERBOSE(9) std::cout << "Set selective_add set input nr: " << i << std::endl;
        ...
        output.clear();
        output = *(parent->sel_add[1].calc->get_output());
        IF_VERBOSE(4) std::cout << "output is:" << (output) << std::endl;

        IF_VERBOSE(4) LEAVE_FUNCTION("calc_rccm_C1::compute(base_obj *parent_")
        return &output;
    }
    const spec_rccm spec = spec_rccm(2);
};


#endif //ADDNET_PERMUTATOR_V2_CALC_RCCM_C2_H
